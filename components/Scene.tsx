...
const TopStar = ({ treeState, config }: { treeState: TreeState, config: ParticleConfig }) => {
  ...
  const { starShape, extrudeSettings } = useMemo(() => {
    ...
    const points = 5;
    const outerRadius = 1.0; // Increased from 0.6
    const innerRadius = 0.4; // Increased from 0.25
  ...
  useFrame((state) => {
    ...
    meshRef.current.rotation.y = time * 1.5; // Faster spin
    meshRef.current.rotation.z = Math.sin(time * 2.0) * 0.15;
    
    const baseScale = treeState === TreeState.GATHERED ? 1.0 : 0.4;
    // Reduced pulse intensity for more subtle size change (was 0.3)
    const pulseIntensity = treeState === TreeState.GATHERED ? 0.1 : 0.05;
    const scale = baseScale + Math.sin(time * 5) * pulseIntensity;
    
    // Adjusted Y from 7.8 to 8.0 to account for larger size
